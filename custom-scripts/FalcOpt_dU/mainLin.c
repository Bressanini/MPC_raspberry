...
void inicializaFalcOpt(int NX, int NU,double** f_x0, double** f_u, double** f_xref, double** f_uref, double** f_x, double** f_fval, unsigned int** f_iter, unsigned int** f_iter_ls, double** f_optimval, double** f_feasval, double** f_meritval){
	...
	*f_x 	 	= (double*		 )malloc(sizeof(double		 )*(NX)*(20));
	*f_u  		= (double*		 )malloc(sizeof(double		 )*(NU)*(20));
	*f_xref  	= (double*		 )malloc(sizeof(double		 )*(NX)*(20));
	*f_uref	 	= (double*		 )malloc(sizeof(double		 )*(NU)*(20));
	...
	*f_meritval	= (double*		 )malloc(sizeof(double		 )			 );
	...
	for (i=0; i<(NX)*(20)	; i++) (*f_x )[i] = 0.0;
	for (i=0; i<(NU)*(20)	; i++) (*f_u )[i] = 0.0;

	for (i=0; i<(NX)*(20)	; i++) (*f_xref )[i] = 0.0;
	for (i=0; i<(NU)*(20)	; i++) (*f_uref )[i] = 0.0;
...
for (k=0; k<NIT;k++){
	...
	for (i=0;i<NX;i++) for (j=0; j<20; j++) f_xref[NX*j+i]  = Xref[NIT*i+k]; // Passando referência de x

	for (i=0;i<NU;i++) for (j=0; j<20; j++) f_uref[NU*j+i]  = Uref[NIT*i+k]; // Passando referência de u
