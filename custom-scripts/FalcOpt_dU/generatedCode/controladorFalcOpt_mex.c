 ...
 *********/
	...
	double timer_getTime(timer* t) {
		return (t->stop.QuadPart - t->start.QuadPart) / (double)(t->freq.QuadPart); }
	void timer_init(timer* t) { /* Initialize frequency */
		QueryPerformanceFrequency(&(t->freq)); }
	void timer_start(timer* t) {
		QueryPerformanceCounter(&(t->start)); }
	double timer_stop(timer* t) {
		QueryPerformanceCounter(&(t->stop));
		return timer_getTime(t);
	};
 ...
 * MEX Function *
 ****************/
static  void controladorFalcOpt_copy(const double* v, double* vt, unsigned int n) {
	unsigned int i;
	for(i=0; i<n; i++) { vt[i] = v[i]; }
};
static  void controladorFalcOpt_UINTtoMATLAB(const unsigned int* v, double* vt, unsigned int n) {
	unsigned int i;
	for(i=0; i<n; i++) { vt[i] = (double)v[i]; }
};

void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	const double* x0 = mxGetPr(prhs[0]);
	const double* xref = mxGetPr(prhs[1]);
	const double* uref = mxGetPr(prhs[2]);
	double u[30];
	int* flag;
	mxArray* nitOut = mxCreateDoubleMatrix(1,1, mxREAL);
	unsigned int nit[1];
	mxArray* lineSearch_nitOut = mxCreateDoubleMatrix(1, CONTROLADOR_FALC_OPT_MAXIMUM_ITERATIONS, mxREAL);
	unsigned int lineSearch_nit[CONTROLADOR_FALC_OPT_MAXIMUM_ITERATIONS];
	double x[90];
	double fval[1];
	double optimval[1];
	double feasval[1];
	double meritval[1];
	mxArray* xOut = mxCreateDoubleMatrix(6,15,mxREAL);
	mxArray* fvalOut = mxCreateDoubleMatrix(1,1,mxREAL);
	mxArray* uOut = mxCreateDoubleMatrix(2,15,mxREAL);
	mxArray* optimvalOut = mxCreateDoubleMatrix(1,1,mxREAL);
	mxArray* feasvalOut = mxCreateDoubleMatrix(1,1,mxREAL);
	mxArray* meritvalOut = mxCreateDoubleMatrix(1,1,mxREAL);
	char** fieldnames;
	...
	if(nrhs < 3 || nrhs > 4) {
		mexErrMsgIdAndTxt("controladorFalcOpt:InvalidInput", "Requires bewteen 3 and 4 inputs."); }
	if(!mxIsDouble(prhs[0]) || mxIsComplex(prhs[0]) || mxGetN(prhs[0]) != 1 || mxGetM(prhs[0]) != 6) {
		mexErrMsgIdAndTxt("controladorFalcOpt:InvalidDimension", "Initial state x0 has invalid dimension. Needs to be a column vector of size 6."); }
	if(!mxIsDouble(prhs[1]) || mxIsComplex(prhs[1]) || mxGetN(prhs[1]) != 1 || mxGetM(prhs[1]) != 90) {
		mexErrMsgIdAndTxt("controladorFalcOpt:InvalidDimension", "State reference vector xref has invalid dimension. Needs to be a column vector of size 15*6."); }
	if(!mxIsDouble(prhs[2]) || mxIsComplex(prhs[2]) || mxGetN(prhs[2]) != 1 || mxGetM(prhs[2]) != 30) {
		mexErrMsgIdAndTxt("controladorFalcOpt:InvalidDimension", "Input reference vector uref has invalid dimension. Needs to be a column vector of size 15*2."); }
	if(nrhs == 4) { /* If a warmstart is supplied */
		if(!mxIsDouble(prhs[3]) || mxIsComplex(prhs[3]) || mxGetN(prhs[3]) != 1 || mxGetM(prhs[3]) != 30) {
			mexErrMsgIdAndTxt("controladorFalcOpt:InvalidDimension", "Warmstart vector uinit has invalid dimension. Needs to be a column vector of size 15*2."); }
		controladorFalcOpt_copy(mxGetPr(prhs[3]), u, 30);
	} else {
		for(i=0; i<30; i++) { u[i] = 0.0; } /* Initialize to zero */
	}
	/* Initialize timer */
	timer_init(&controladorFalcOpt_T);

	/* Prepare outputs */
	if(nlhs < 1 || nlhs > 3){
		mexErrMsgIdAndTxt("controladorFalcOpt:InvalidOutput", "Requires bewteen 1 and 3 outputs."); }
	if(nlhs >= 2) {
		plhs[1] = mxCreateNumericMatrix(1,1, mxINT32_CLASS, mxREAL);
		flag = (int*)mxGetData(plhs[1]);
	} else { 
		flag = (int*)mxMalloc(sizeof(int));
	}

	/* Run optimization algorithm */
	timer_start(&controladorFalcOpt_T); /* Start timer */
	flag[0] = proposed_algorithm(x0, u, xref, uref, x, fval, nit, lineSearch_nit, optimval, feasval, meritval);
	timer_stop(&controladorFalcOpt_T); /* Stop timer */

	/* Process outputs */
	plhs[0] = mxCreateDoubleMatrix(2,1,mxREAL);
	controladorFalcOpt_copy(u, mxGetPr(plhs[0]), 2);
	controladorFalcOpt_UINTtoMATLAB(nit, mxGetPr(nitOut), 1);
	controladorFalcOpt_UINTtoMATLAB(lineSearch_nit, mxGetPr(lineSearch_nitOut), CONTROLADOR_FALC_OPT_MAXIMUM_ITERATIONS);
	controladorFalcOpt_copy(x, mxGetPr(xOut), 90);
	controladorFalcOpt_copy(fval, mxGetPr(fvalOut), 1);
	controladorFalcOpt_copy(u, mxGetPr(uOut), 30);
	controladorFalcOpt_copy(optimval, mxGetPr(optimvalOut), 1);
	controladorFalcOpt_copy(feasval, mxGetPr(feasvalOut), 1);
	controladorFalcOpt_copy(meritval, mxGetPr(meritvalOut), 1);
	if(nlhs >= 3) {
		fieldnames = mxCalloc(9, sizeof(char*)); /* Allocate memory for storing pointers */
		fieldnames[0] = (char*)mxMalloc(sizeof(char)*11);
		memcpy(fieldnames[0],"iterations",sizeof("iterations"));
		fieldnames[1] = (char*)mxMalloc(sizeof(char)*11);
		memcpy(fieldnames[1],"lineSearch",sizeof("lineSearch"));
		fieldnames[2] = (char*)mxMalloc(sizeof(char)*11);
		memcpy(fieldnames[2],"time",sizeof("time"));
		fieldnames[3] = (char*)mxMalloc(sizeof(char)*11);
		memcpy(fieldnames[3],"u",sizeof("u"));
		fieldnames[4] = (char*)mxMalloc(sizeof(char)*11);
		memcpy(fieldnames[4],"x",sizeof("x"));
		fieldnames[5] = (char*)mxMalloc(sizeof(char)*11);
		memcpy(fieldnames[5],"fval",sizeof("fval"));
		fieldnames[6] = (char*)mxMalloc(sizeof(char)*11);
		memcpy(fieldnames[6],"optimval",sizeof("optimval"));
		fieldnames[7] = (char*)mxMalloc(sizeof(char)*11);
		memcpy(fieldnames[7],"feasval",sizeof("feasval"));
		fieldnames[8] = (char*)mxMalloc(sizeof(char)*11);
		memcpy(fieldnames[8],"meritval",sizeof("meritval"));
		plhs[2] = mxCreateStructMatrix(1,1,9,(const char**)fieldnames);
		mxFree(fieldnames[0]);
		mxFree(fieldnames[1]);
		mxFree(fieldnames[2]);
		mxFree(fieldnames[3]);
		...
		mxFree(fieldnames[7]);
		mxFree(fieldnames[8]);
		mxSetFieldByNumber(plhs[2],0,0,nitOut); /* Number of iterations */
		fieldnames[0] = (char*)mxMalloc(sizeof(char)*11);
		memcpy(fieldnames[0],"iterations",sizeof("iterations"));
		lsStruct = mxCreateStructMatrix(1,1,1,(const char**)fieldnames);
		mxFree(fieldnames[0]);
		mxSetFieldByNumber(lsStruct,0,0,lineSearch_nitOut);
		mxSetFieldByNumber(plhs[2],0,1,lsStruct);
		mxFree(fieldnames);
		mxGetPr(t)[0] = timer_getTime(&controladorFalcOpt_T);
		mxSetFieldByNumber(plhs[2],0,2,t); /* Runtime */
		mxSetFieldByNumber(plhs[2],0,3,uOut); /* u */
		mxSetFieldByNumber(plhs[2],0,4,xOut); /* x */
		mxSetFieldByNumber(plhs[2],0,5,fvalOut); /* fval */
		mxSetFieldByNumber(plhs[2],0,6,optimvalOut); /* optimval */
		mxSetFieldByNumber(plhs[2],0,7,feasvalOut); /* feasval */
		mxSetFieldByNumber(plhs[2],0,8,meritvalOut); /* meritval */
	}
	if(nlhs < 2) { mxFree(flag); }
}
